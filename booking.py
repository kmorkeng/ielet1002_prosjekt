...
# Lager en indeks og en liste over alle minuttene rommet er booket
...
def startAndEnd(hour, minute, duration):
    ...
    separator = ":"
    seconds = "00"
    hourminute = hour + separator + minute + separator + seconds
    today = datetime.now().date()
    starttime1 = datetime.strptime(hourminute, "%H:%M:%S")
    start = datetime.combine(today, datetime.time(starttime1))
    end = start + dt.timedelta(minutes = duration)
    return start, end

# Lager en dataframe med bookingen som ble hentet fra CoT
def create_entrydf(user, startdt, enddt):
    timelist, indexcol = create_timelist(startdt, enddt)
    entrydf = pd.DataFrame(columns = columns2)
...
def statusCheck(dbdf, entrydf, databasenum, startdt, enddt):
    ...
    max_value = check.max()
    max_value = len(str(max_value))
    limit = max_amount_col(databasenum)
    if max_value >= limit:
        return 0
...
def accept_fastlane(dbdf, entrydf, databasenum, startdt, enddt):
    df = dbdf.join(entrydf, how = 'outer')
    ...
    index_1 = df.index
    df0 = df
    df0['user1'] = df[startdt:enddt].apply(f, axis = 1)
    df0.loc[df0['user1'].isnull(),'user1'] = df0['user']
    df1 = df0.loc[:, ['user1']]
...
def format_values(starttime, startminutt, varighet, bruker, rom):
    startdt, enddt = startAndEnd(starttime, startminutt, varighet)
    entrydf, timelist = create_entrydf(bruker, startdt, enddt)
    dbdf = check_db(rom)
...
def check_intersection(df1, timelist):
    if df1.empty:
        ...
        common = intersection(timelist1, dblist1)
        howoften = int(len(common))
        return howoften
    
# Sletter alle bookinger som er tidsstemplet for mer en 24 timer siden
def delete_old(df):
    if df.empty:
        return df
    else:
        ...
        df_new.drop(df_new.loc[df_new.index < today].index, inplace=True)
        return df_new
...
while True:
    hey = wassup()
    if hey == 1:
        bruker, rom, varighet, startminutt, starttime = find_value()
        startdt, enddt, entrydf, dbdf, timelist = format_values(starttime, startminutt, varighet, bruker, rom)
        delete_old(dbdf)
        what = check_intersection(dbdf, timelist)
        if what >= 1:
            status = statusCheck(dbdf, entrydf, rom, startdt, enddt)
            if status == 0:
                deny()
            elif status == 1:
